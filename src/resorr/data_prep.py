...
# aggregate
def aggregate(ds, frequency='weekly', operations={'unregulated_inflow': np.mean, 'storage_change': np.sum}):
    if frequency == 'daily':
        resampled = ds
        resampled['dt'] = ds['time'].resample(time='1D').count()
    elif frequency == 'weekly':
        resampled = ds.resample(time='1W')
        for key, op in operations.items():
            resampled[key] = ds[key].resample(time='1W').apply(op)
        resampled['dt'] = ds['time'].resample(time='1W').count()
    elif frequency == 'biweekly':
        resampled = ds.resample(time='2W')
        for key, op in operations.items():
            resampled[key] = ds[key].resample(time='2W').apply(op)
        resampled['dt'] = ds['time'].resample(time='2W').count()
    elif frequency == 'monthly':
        resampled = ds.resample(time='1M')
        for key, op in operations.items():
            resampled[key] = ds[key].resample(time='1M').apply(op)
        resampled['dt'] = ds['time'].resample(time='1M').count()
    elif frequency == 'annual':
        resampled = ds.resample(time='1Y')
        for key, op in operations.items():
            resampled[key] = ds[key].resample(time='1Y').apply(op)
...
def calculate_volumes(
    ...
    # all_vars_except_fluxes = list(set(all_vars) - set(fluxes))
    for flux in fluxes:
